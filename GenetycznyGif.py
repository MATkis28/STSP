...
def genetyczny(graf):
    ...
    while(True):
        ...
        if (sols[0] != prevSol):
            prevSol = sols[0]
            rysujRozwiazanie(prevSol, "Generacja " + str(i))
            saveplot(j)
            j += 1
            if (j == 15):
                break
...
def saveplot(i):
    plot.savefig(str(n) + 'n' + str(populacja)+'pop'+str(i)+'.png')
    plot.clf()
    #plot.show()

def savegif():
    with imageio.get_writer(str(n) + 'n' + str(populacja)+'pop.gif', mode='I') as writer:
        for i in range(15):
            image = imageio.imread(str(n)   + 'n' + str(populacja)+'pop' +str(i)+'.png')
            writer.append_data(image)
        for _ in range(15):
            image = imageio.imread(str(n) + 'n' + str(populacja)+'pop14.png')
            writer.append_data(image)
            
kalibrujAlgorytm(160, 40, 1)
...
graf = generujGrafNaPlaszczyznie(n, punkty, skalary)
genetyczny(graf)
savegif()
